...
export default (state = initialState, action) => {
  ...
  } else if (type === `${ORDER_CONSTANTS.GET_ORDERS_BY_USER}/${SHARED_CONSTANTS.REQUEST}`) {

    const {isPending} = action.payload;
    state = {...state, isPending};

  } else if (type === `${ORDER_CONSTANTS.GET_ORDERS_BY_USER}/${SHARED_CONSTANTS.SUCCESS}`) {

    const {isPending, orders} = action.payload;
    state = {...state, isPending, items: orders};

  } else if (type === `${ORDER_CONSTANTS.GET_ORDERS_BY_USER}/${SHARED_CONSTANTS.FAILURE}`) {
